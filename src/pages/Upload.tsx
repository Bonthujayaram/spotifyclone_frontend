...
const Upload = () => {
  ...
  const validateAudio = (file: File): boolean => {
    ...
    if (file.size > MAX_AUDIO_MB * 1024 * 1024) {
      toast({
        ...
        description: `Audio must be ${MAX_AUDIO_MB}MB or less.`,
        variant: 'destructive',
      });
      return false;
    }
  ...
  const handleDrop = (e: DragEvent) => {
    ...
    if (files.length > 0 && validateAudio(files[0])) {
  ...
  const handleFileSelect = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0 && validateAudio(files[0])) {
      setUploadedFile(files[0]);
    }
  };
    ...
    if (files && files.length > 0) {
      ...
      if (file.size > MAX_IMAGE_MB * 1024 * 1024) {
        toast({
          ...
          description: `Cover image must be ${MAX_IMAGE_MB}MB or less.`,
          variant: 'destructive',
        });
        return;
      }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!uploadedFile) {
      toast({
        ...
        description: 'Select an audio file before uploading.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (!token) {
      toast({
        ...
        description: 'Please sign in before uploading tracks.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      setIsUploading(true);
      const [audioDataUrl, duration] = await Promise.all([
        readFileAsDataUrl(uploadedFile),
        readAudioDuration(uploadedFile),
      ]);

      await authApi.createCreatorTrack(token, {
      ...
      toast({
        ...
        description: 'Your track is now available in Creator Studio.',
      });
      ...
      toast({
        title: 'Upload failed',
        description: error instanceof Error ? error.message : 'Unable to upload track.',
        variant: 'destructive',
      });
    } finally {
    ...
    <div className="p-6 max-w-4xl mx-auto animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <CardContent>
            {!uploadedFile ? (
              ...
              >
                ...
                <Button type="button" variant="outline" className="cursor-pointer" onClick={openAudioPicker}>
                  Choose File
                </Button>
                <input
                  ref={audioInputRef}
                ...
                />
                <p className="text-gray-500 text-xs mt-4">Supported formats: MP3, WAV, FLAC (Max 12MB)</p>
              </div>
            ) : (
              <div className="bg-white/5 rounded-lg p-4 flex items-center gap-4">
                ...
                {audioPreviewUrl ? (
                  <audio controls src={audioPreviewUrl} className="h-9 max-w-[200px]" />
                ) : (
                  ...
                  {!coverImage ? (
                    <>
                      <button
                        ...
                        className="w-full aspect-square border-2 border-dashed border-gray-600 rounded-lg flex items-center justify-center cursor-pointer hover:border-gray-500 transition-colors"
                      >
        ...
        <div className="flex justify-end gap-4">
          <Button variant="outline" type="button" onClick={() => navigate('/creator-studio')}>
            Save as Draft
          </Button>
